<template>
        ...
        <div class="flex flex-col space-y-4">
          <div class="flex justify-between items-center">
            ...
            <div class="flex justify-center items-center cursor-pointer"
              @click="expense.category === null && toggleIsCategoryOpen()">
              <h4 class="h4">
                {{
                  expense.category
          ...
          <div class="flex justify-between items-center">
            ...
            <div class="flex justify-center items-center cursor-pointer" @click="toggleIsMemoOpen">
              <h4 class="h4">
                {{
  ...
  <div v-show="isCategoryOpen"
    class="absolute w-full bottom-0 z-30 shadow-[0px_8px_14px_2px_rgba(0,_0,_0,_0.35)] rounded-t-3xl bg-gray-200 h-150">
    ...
    <div class="w-full p-8 pt-6">
      <div class="grid grid-cols-2 gap-5.5 pt-5.5 pb-13">
      ...
      <button @click="toggleIsCategoryOpen"
        class="text-cocoa-600 focus:ring-1 rounded-t-lg text-sm w-full text-center py-1.5 bg-gold-300 border border-cocoa-200 h3">확인</button>
    </div>
  </div>
  ...
  <div v-show="isMemoOpen"
    class="absolute w-full bottom-0 z-30 shadow-[0px_8px_14px_2px_rgba(0,_0,_0,_0.35)] rounded-t-3xl bg-gray-200 h-100">
    <div class="flex w-full items-center px-8 pt-8">
      <div class="flex-1"></div>
    ...
    <div class="w-full p-8">
      <form @submit.prevent="saveDescription">
        ...
        <button type="submit"
          class="text-cocoa-600 focus:ring-1 rounded-t-lg text-sm w-full text-center py-1.5 bg-gold-300 border border-cocoa-200 h3 mb-10">확인</button>
      </form>
    </div>
  </div>
...
const toggleIsCategoryOpen = async () => {
  if (isCategoryOpen.value && selectedCategory.value) {
    const confirmResult = confirm('카테고리를 변경하시겠습니까? 한 번 설정하면 수정할 수 없습니다.')
    if (confirmResult) {
      try {
        await expenseService.updateExpenseCategory(expenseId, selectedCategory.value)
        const newCategory = categories.value.find(
          c => c.category_id === selectedCategory.value
        )

        if (expense.value && newCategory) {
          expense.value.category = newCategory
        }
      } catch (error) {
